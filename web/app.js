...
let lang = storedLang === "zh" || storedLang === "en"
    ? storedLang
...
function showAuth(mode) {
    ...
    const title = $("#authTitle"), hint = $("#authHint"), btn = $("#authBtn"), sw = $("#authSwitch");
    if (mode === "setup") {
...
function setForm(f) {
    for (const k of FIELDS)
        fieldEl(k).value = (f && f[k]) || "";
    $("#f-smartWhitespace").checked = f ? !!f.smartWhitespace : true;
    $("#f-reverse").checked = f ? !!f.reverse : false;
    $("#f-maxItems").value = String((f && f.maxItems) || 25);
    $("#f-ttlMinutes").value = String((f && f.ttlMinutes) || 30);
}
function collectForm() {
    const out = {};
    for (const k of FIELDS)
        out[k] = fieldEl(k).value;
...
function renderPreview(resp, includedPage) {
    ...
    if (resp.globalError) {
        ...
        resp.globalCaptures.forEach((c, i) => {
            ...
            chip.appendChild(document.createTextNode(c.length > 120 ? c.slice(0, 120) + "…" : c));
            g.appendChild(chip);
        });
        show(g);
    }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    $("#regToggle").addEventListener("change", async () => {
        const box = $("#regToggle");
        try {
            const s = await api("/api/admin/settings", { method: "PUT", body: { registrationEnabled: box.checked } });
    ...
    $("#f-sourceUrl").addEventListener("keydown", (e) => {
        if (e.key === "Enter")
            runPreview({ includePage: true, force: true });
    });
    await refreshConfig();
    me = await fetchMe();
    try {
