/* FeedForge UI — TypeScript source. web/app.js is generated from this
   file; edit here and run `npm run build`. */
"use strict";
...
const storedLang = localStorage.getItem("ff_lang");
let lang: Lang = storedLang === "zh" || storedLang === "en"
  ? storedLang
...
// The UI is a static page, so looked-up elements always exist; the cast
// spares a null check at every call site.
function $<T extends Element = HTMLElement>(sel: string): T {
  return document.querySelector(sel) as T;
}

function show(el: HTMLElement, on = true): void { el.hidden = !on; }
...
function toast(msg: string): void {
  ...
  toastTimer = setTimeout(() => show(el, false), 2400);
}
...
async function api<T>(path: string, opts: ApiOptions = {}): Promise<T> {
  ...
  if (res.status === 401) {
    throw new ApiError((data && data.error) || t("sessionExpired"), true);
  }
  if (!res.ok) {
    throw new ApiError((data && data.error) || res.statusText);
  }
  return data as T;
}
...
let config: AppConfig = { needsSetup: false, registrationEnabled: false };
let me: UserInfo | null = null;             // signed-in user, or null
let authMode: AuthMode = "login";
let editingId: string | null = null;        // feed being edited, or null for a new feed
let pendingPrefill: Partial<Feed> | null = null; // form values to apply when the editor opens
...
async function fetchMe(): Promise<UserInfo | null> {
  try {
    return await api<UserInfo>("/api/auth/me");
  } catch (_) {
    return null;
  }
}
...
async function route(): Promise<void> {
  ...
  if (hash === "#/new") {
    ...
    try {
      const f = await api<Feed>("/api/feeds/" + encodeURIComponent(id));
      editingId = f.id;
      openEditor(f);
    } catch (e) {
      handleAuthError(e);
...
function showAuth(mode: AuthMode): void {
  ...
  const title = $("#authTitle"), hint = $("#authHint"),
    btn = $<HTMLButtonElement>("#authBtn"), sw = $<HTMLAnchorElement>("#authSwitch");
  if (mode === "setup") {
  ...
  $<HTMLInputElement>("#a-password").autocomplete =
    mode === "login" ? "current-password" : "new-password";
  $<HTMLInputElement>("#a-username").focus();
}

async function submitAuth(): Promise<void> {
  const username = $<HTMLInputElement>("#a-username").value.trim();
  const password = $<HTMLInputElement>("#a-password").value;
  const path = authMode === "login" ? "/api/auth/login" : "/api/auth/register";
  const btn = $<HTMLButtonElement>("#authBtn");
  btn.disabled = true;
  try {
    me = await api<UserInfo>(path, { method: "POST", body: { username, password } });
    $<HTMLInputElement>("#a-password").value = "";
    await refreshConfig();
    if (location.hash && location.hash !== "#/") location.hash = "#/";
    route();
  } catch (e) {
    $("#authError").textContent = errMsg(e);
    show($("#authError"));
  } finally {
    btn.disabled = false;
  }
...
async function renderList(): Promise<void> {
  ...
  let feeds: Feed[] = [];
  try {
    feeds = (await api<Feed[]>("/api/feeds")) || [];
  } catch (e) {
    handleAuthError(e);
    return;
  }
...
async function renderAdminPanel(): Promise<void> {
  ...
  try {
    ...
    $<HTMLInputElement>("#regToggle").checked = !!s.registrationEnabled;
  } catch (_) { /* leave as-is */ }
}

function feedCard(f: Feed): HTMLDivElement {
...
function handleAuthError(e: unknown): void {
  toast(errMsg(e));
  if (e instanceof ApiError && e.auth) {
    me = null;
...
const FIELDS = ["sourceUrl", "encoding", "globalPattern", "itemPattern",
  "title", "link", "description", "itemTitle", "itemLink", "itemContent"] as const;

type FieldKey = (typeof FIELDS)[number];

function fieldEl(k: FieldKey): FormControl {
  return $<FormControl>("#f-" + k);
}

function setForm(f: Partial<Feed> | null): void {
  for (const k of FIELDS) fieldEl(k).value = (f && f[k]) || "";
  $<HTMLInputElement>("#f-smartWhitespace").checked = f ? !!f.smartWhitespace : true;
  $<HTMLInputElement>("#f-reverse").checked = f ? !!f.reverse : false;
  $<HTMLInputElement>("#f-maxItems").value = String((f && f.maxItems) || 25);
  $<HTMLInputElement>("#f-ttlMinutes").value = String((f && f.ttlMinutes) || 30);
}

function collectForm(): FeedForm {
  const out = {} as FeedForm;
  for (const k of FIELDS) out[k] = fieldEl(k).value;
  out.sourceUrl = out.sourceUrl.trim();
  out.smartWhitespace = $<HTMLInputElement>("#f-smartWhitespace").checked;
  out.reverse = $<HTMLInputElement>("#f-reverse").checked;
  out.maxItems = parseInt($<HTMLInputElement>("#f-maxItems").value, 10) || 25;
  out.ttlMinutes = parseInt($<HTMLInputElement>("#f-ttlMinutes").value, 10) || 30;
  return out;
}

function openEditor(feedOrNull: Partial<Feed> | null): void {
  show($("#view-editor"));
  ...
  if ($<HTMLInputElement>("#f-sourceUrl").value.trim()) {
...
async function runPreview(
  { includePage = false, force = false }: { includePage?: boolean; force?: boolean },
): Promise<void> {
  ...
  try {
    resp = await api<PreviewResponse>("/api/preview", { method: "POST", body: req });
  } catch (e) {
...
function renderPreview(resp: PreviewResponse, includedPage: boolean): void {
  // fetch status
  if (resp.fetchError) {
...
function renderItemsTable(resp: PreviewResponse): void {
...
function renderFeedPreview(resp: PreviewResponse): void {
  const items = resp.items || [];
...
async function saveFeed(): Promise<void> {
  ...
  try {
    let saved: Feed;
    if (editingId) {
      saved = await api<Feed>("/api/feeds/" + editingId, { method: "PUT", body: form });
    } else {
      saved = await api<Feed>("/api/feeds", { method: "POST", body: form });
      editingId = saved.id;
    ...
    $<HTMLAnchorElement>("#savedRssOpen").href = rss;
    show($("#savedPanel"));
    $("#savedPanel").scrollIntoView({ behavior: "smooth", block: "center" });
  } catch (e) {
    $("#saveStatus").textContent = `${t("saveFailed")}: ${errMsg(e)}`;
    handleAuthError(e);
  } finally {
    btn.disabled = false;
  }
...
function useRecipe(r: Recipe): void {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $<HTMLInputElement>("#regToggle").addEventListener("change", async () => {
    const box = $<HTMLInputElement>("#regToggle");
    try {
      const s = await api<AdminSettings>("/api/admin/settings",
